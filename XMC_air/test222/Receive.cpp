...
extern  int32_t pressre_air;
extern volatile int16_t baro_alt_air_ref;
extern volatile int16_t baro_alt_air;
extern volatile  int8_t  baro_h_offset;
extern volatile  int8_t  baro_delta_h;
...
TerminalCommand Terminal_ground;
...
void TerminalCommand::terminalCommandHandler(void)
{
	...
	switch(cmdIn[2])
	{
		case 0x00:       //ground disable h_hold
			send_flag = 0;
			baro_h_offset = (int8_t)cmdIn[4];
		  break;
		case 0x01:       //ground  enable h_hold
			send_flag = 1;
      baro_h_offset = (int8_t)cmdIn[4];
			break;
    case 0x03:       // set air alt ref
			send_flag = 0;
			baro_alt_air_ref = baro_alt_air;
			baro_h_offset = (int8_t)cmdIn[4];
		  break;
		case 0x10:       //雷达数据
...
void USIC1_5_IRQHandler(void)     //RADAR Communication
{
   	uint8_t oneByte = XMC_UART_CH_GetReceivedData(radar_uart);
	  Data_Receive_Prepare(&Terminal_radar,oneByte);
}

void USIC2_4_IRQHandler(void)    //STM Communication           //radar/stm channel exchanged
{
	  uint8_t oneByte = XMC_UART_CH_GetReceivedData(stm_uart); 
	  Data_Receive_Prepare(&Terminal_stm,oneByte);
}
void USIC2_5_IRQHandler(void)     //GROUND Communication
{
	  uint8_t oneByte = XMC_UART_CH_GetReceivedData(ground_uart);
	   Data_Receive_Prepare(&Terminal_ground,oneByte);
