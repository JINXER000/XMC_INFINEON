...
#define STM_UART_RX_PIN      P0_4 
#define STM_UART_TX_PIN      P0_5       //U1C0     U1C1 used by I2C (DPS310)

#define AIR_UART_RX_PIN      P3_7       //P5_1
#define AIR_UART_TX_PIN      P3_8     //U2C0    P5_0


#define RADAR_UART_RX_PIN      P3_12    
#define RADAR_UART_TX_PIN      P3_11     //U2C1
...
XMC_USIC_CH_t           *stm_uart = XMC_UART1_CH0;
...
XMC_USIC_CH_t           *radar_uart = XMC_UART2_CH1;
...
/* air_UART configuration */
XMC_USIC_CH_t           *air_uart = XMC_UART2_CH0;
XMC_UART_CH_CONFIG_t     air_uart_config =
{
		115200U,
		8U,
		8U,
...
XMC_GPIO_CONFIG_t       air_rx_pin_config;
XMC_GPIO_CONFIG_t       air_tx_pin_config;
...
void STM_UART_Config(void)
{
  ...
  XMC_UART_CH_SetInputSource(stm_uart, XMC_UART_CH_INPUT_RXD , USIC1_C0_DX0_P0_4);
  XMC_UART_CH_Start(stm_uart);  // uart.CCR.mode=UART-> USIC1_CH0 is switched for UART operation mode
  ...
  stm_tx_pin_config.mode = XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT2;  // Tx output in ALT2
...
void RADAR_UART_Config(void)
{
  ...
  XMC_UART_CH_SetInputSource(radar_uart, XMC_UART_CH_INPUT_RXD , USIC2_C1_DX0_P3_12);
  ...
  XMC_USIC_CH_SetInterruptNodePointer(radar_uart, XMC_USIC_CH_INTERRUPT_NODE_POINTER_RECEIVE, 4);             // set USIC0 SR5 for RI
  XMC_USIC_CH_EnableEvent(radar_uart, XMC_USIC_CH_EVENT_ALTERNATIVE_RECEIVE);                                         // 'USIC_AI.007' -> AIR ON
  XMC_USIC_CH_SetInterruptNodePointer(radar_uart, XMC_USIC_CH_INTERRUPT_NODE_POINTER_ALTERNATE_RECEIVE, 4);

  NVIC_SetPriority(USIC2_4_IRQn,4);//NVIC_EncodePriority(NVIC_GetPriorityGrouping(),63U, 0U));   // CMSIS function for NVIC control: NVIC_SetPriority(IRQn_t IRQn, uint32_t priority): priority=0..63
  NVIC_EnableIRQ(USIC2_4_IRQn);       // CMSIS function for NVIC control: enable IRQn

}	

void AIR_UART_Config(void)
{

  XMC_UART_CH_Init(air_uart, &air_uart_config);
  XMC_UART_CH_SetInputSource(air_uart, XMC_UART_CH_INPUT_RXD , USIC2_C0_DX0_P3_7);
  XMC_UART_CH_Start(air_uart);  // uart.CCR.mode=UART-> USIC2_CH0 is switched for UART operation mode


  /*air_UART Rx pin configuration*/
  air_rx_pin_config.mode = XMC_GPIO_MODE_INPUT_PULL_UP;          // Rx input with internal pull-up
  XMC_GPIO_Init(AIR_UART_RX_PIN, &air_rx_pin_config);
  /*air_UART Tx pin configuration*/
  air_tx_pin_config.output_level = XMC_GPIO_OUTPUT_LEVEL_HIGH;
  air_tx_pin_config.mode = XMC_GPIO_MODE_OUTPUT_PUSH_PULL_ALT1;  // Tx output in ALT2
  XMC_GPIO_Init(AIR_UART_TX_PIN, &air_tx_pin_config);
	
	
  XMC_USIC_CH_EnableEvent(air_uart, XMC_USIC_CH_EVENT_STANDARD_RECEIVE);                                            // enable interrupt RI
  XMC_USIC_CH_SetInterruptNodePointer(air_uart, XMC_USIC_CH_INTERRUPT_NODE_POINTER_RECEIVE, USIC0_SR5);             // set USIC0 SR5 for RI
  XMC_USIC_CH_EnableEvent(air_uart, XMC_USIC_CH_EVENT_ALTERNATIVE_RECEIVE);                                         // 'USIC_AI.007' -> AIR ON
  XMC_USIC_CH_SetInterruptNodePointer(air_uart, XMC_USIC_CH_INTERRUPT_NODE_POINTER_ALTERNATE_RECEIVE, USIC0_SR5);
...
void UARTxNVIC_Config()
{
  ...
  XMC_USIC_CH_SetInterruptNodePointer(stm_uart, XMC_USIC_CH_INTERRUPT_NODE_POINTER_RECEIVE, USIC0_SR5);             // set USIC0 SR5 for RI
  XMC_USIC_CH_EnableEvent(stm_uart, XMC_USIC_CH_EVENT_ALTERNATIVE_RECEIVE);                                         // 'USIC_AI.007' -> AIR ON
  XMC_USIC_CH_SetInterruptNodePointer(stm_uart, XMC_USIC_CH_INTERRUPT_NODE_POINTER_ALTERNATE_RECEIVE, USIC0_SR5);
